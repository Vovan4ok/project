...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests().antMatchers("/").permitAll().antMatchers("/home").access("hasRole('ROLE_USER') or hasRole('ROLE_ADMIN')").
                antMatchers("/userFaculties", "/application", "/listOfApplicants").access("hasRole('ROLE_USER')")
                .antMatchers("/adminFaculties", "/acceptApplication", "/addFaculty", "/update", "/delete").access("hasRole('ROLE_ADMIN')")
